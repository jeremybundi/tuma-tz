...
const Payment = () => {
  ...
  return (
      ...
      <div className="bg-[#161C28] text-slate-200 p-8 md:p-12 md:px-24  rounded-2xl md:container mx-auto w-full"> {/* Adjusted padding for consistency */}
        {/* Top Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-12 lg:gap-16 mb-6 md:mb-10"> {/* Increased bottom margin */}
          {/* Left Column */}
          <div className="md:col-span-1">
            <p className="text-[12px] md:text-[16px] lg:text-[16px] font-semibold uppercase tracking-wider text-[#D0D5DD] mb-2">
              ABOUT US
            </p>
            <span className="block text-[24px] md:text-[36px] font-bold text-white leading-tight md:mb-6 mb-1">
                <p>
                A Licensed Payment
                </p>
                <p>
          ...
          {/* Right Column */}
          <div className="md:col-span-1 text-gray-300 text-[12px] md:text-[14px] sm:text-base leading-relaxed space-y-4">
            <p>
        ...
        <div>
          <h2 className="text-[18px] md:text-[24px] font-semibold text-[#D0D5DD] mb-4 md:mb-6"> {/* Increased bottom margin */}
            The Tuma Advantage
          </h2>
          {/* Container for advantage cards: flex row on small, grid on medium+ */}
          <div className="flex overflow-x-auto space-x-6 pb-4 md:grid md:grid-cols-3 md:gap-x-16 lg:gap-x-20 md:gap-y-8 md:space-x-0 md:pb-0 md:overflow-visible">
            {advantages.map((advantage, index) => (
              <div
                key={index}
                // Card styling: fixed width and no-shrink for small screens, auto width for grid
                className="bg-[#0B0B0B] p-8 rounded-xl flex flex-col w-72 sm:w-80 flex-shrink-0 md:w-auto"
              >
